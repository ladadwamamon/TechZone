...
## 9. المراقبة والأداء (Monitoring)

يوفّر المتجر مراقبة مدمجة عبر صفحة "الأداء والمراقبة" في لوحة التحكم، إضافةً إلى خيارات خارجية اختيارية.

### المراقبة المدمجة (داخل اللوحة)
- صفحة `/performance` في لوحة التحكم تعرض مباشرةً: مدة التشغيل، عدد الطلبات، أخطاء الخادم (آخر 50)، استهلاك الذاكرة (Heap/RSS)، وإصدار Node.
- مؤشرات الويب الحيوية (Core Web Vitals: LCP, INP, CLS, FCP, TTFB) تُجمع تلقائياً من زيارات المتجر الحقيقية وتُخزَّن في جدول `web_vitals`، وتُعرض كقيم p75 ومتوسط لآخر 7 أيام.
- لا تتطلب أي إعداد إضافي — تعمل تلقائياً بعد النشر.

### Sentry (تتبع الأخطاء — اختياري)
لتفعيل تتبع الأخطاء المتقدم مع تنبيهات:
1. أنشئ مشروعاً على https://sentry.io واحصل على الـ DSN.
2. أضف متغير البيئة `SENTRY_DSN` إلى خدمة الـ API (وعند الرغبة `VITE_SENTRY_DSN` للواجهة).
3. عند ضبط المتغير يمكن لاحقاً ربط Sentry SDK؛ في حال عدم ضبطه يبقى المتجر يعمل بالمراقبة المدمجة فقط.

### مراقبة التوافر الخارجية (Uptime)
- **UptimeRobot** (مجاني): أضف مراقباً من نوع HTTP(s) على `https://<domain>/api/healthz` بفاصل 5 دقائق، مع تنبيهات بريد/تليجرام عند التوقف.
- **Cloudflare**: فعّل Health Checks (في الخطط المدفوعة) أو استخدم Analytics وNotifications للتنبيه عند ارتفاع الأخطاء 5xx.

### نقطة الفحص الصحي
- `GET /api/healthz` تُرجع حالة الخدمة وتُستخدم من Docker healthcheck وأدوات المراقبة الخارجية.
